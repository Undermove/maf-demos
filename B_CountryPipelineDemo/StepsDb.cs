...
public sealed partial class StepsDb(string path)
{
    ...
    [
        (
            "Выяснить требования для открытия страны",
            """
            Открываем новую страну: **Грузия**, код страны `ge`.

            Что нужно сделать:

            - Передвинуть карточку в `in-progress`.
            - Спросить у менеджера страны, какие языки нужны для локализации и в какой валюте будут цены.
            - Привести ответ к кодам ISO: языки — двухбуквенные (например, «грузинский и английский» → `ka`, `en`),
              валюта — трёхбуквенная (например, «лари» → `GEL`).
            - Записать коды языков и валюты комментарием в эту карточку.
        ...
        (
            ...
            - Добавить `countries/<код страны>.json` строго по структуре образца.
              Код страны, языки и валюту НЕ спрашивать у менеджера: они уже выяснены в карточке #19
              «Выяснить требования для открытия страны» — код страны в её тексте, языки и валюта
              в комментарии к ней. Прочитай её. Поле `status` — `opening`.
            - Передвинуть карточку в `done`.
            """
        ),
        (
